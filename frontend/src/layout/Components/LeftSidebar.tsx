...
import { HomeIcon, LibraryIcon, MessageSquareText } from "lucide-react"
            ...
            <div className="flex-1 rounded bg-zinc-900 p-4">
                <div className="flex items-center justify-between mb-4 ">
                    <div className="flex items-center text-white px-2">
                        ...
                        <span className="hidden md:inline">Paylists</span>
                    </div>
                </div>
                <ScrollArea className='h-[calc(100vh-300px)] rounded'>
                    <div className="space-y-2"> {isLoading ? <PlaylistSkeleton /> : ( 
                            albums.map((album) => (
                                <Link to={`/albums/${album._id}`}
                                key={album._id}
                                className="p-2 hover:bg-zinc-800 rounded flex items-center gap-3 group cursor-pointer">
